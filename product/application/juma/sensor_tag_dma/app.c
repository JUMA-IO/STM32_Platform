...
static void app_sensor_data_print(void* arg)
{
    uint8_t tmp_buf[6] = {1};
    
    imu_sensor_data_t* data = arg;
    
    tmp_buf[0] = (((int32_t)data->acc[0]) >> 8) & 0xFF;
    tmp_buf[1] = ((int32_t)data->acc[0]) & 0xFF;
    tmp_buf[2] = (((int32_t)data->acc[1]) >> 8) & 0xFF;
    tmp_buf[3] = ((int32_t)data->acc[1]) & 0xFF;
    tmp_buf[4] = (((int32_t)data->acc[2]) >> 8) & 0xFF;
    tmp_buf[5] = ((int32_t)data->acc[2]) & 0xFF;
    
    ble_device_send(0x01, 6, tmp_buf);
//    printf("app_sensor_data: %f,%f,%f,%f,%f,%f\n",data->acc[0],data->acc[1],data->acc[2],data->gyro[0],data->gyro[1],data->gyro[2]);
}

/*received data callback*/
void on_imu_sensor_data(imu_sensor_data_t* data)
{
     //app_sensor_data_print(data);
    app_pitch_roll(data);
}
...
void app_print_test(void* arg)
{
    uint8_t* temp = arg;

    switch(*temp){
       case 0: printf("imu_sensor_dma_read_call_back\n");    break;
       case 1: printf("imu_sensor_magneto_irq_callback\n");  break;
    }
}
