...
void complementary_filter(int32_t acc_data[3], int32_t gyr_data[3], int32_t mag_data[3], float *pitch, float *roll, float *yal)
{
    ...
    *pitch += ((float)gyr_data[0] / GYROSCOPE_SENSITIVITY) * dt; // Angle around the X-axis
    //*roll -= ((float)gyr_data[1] / GYROSCOPE_SENSITIVITY) * dt;    // Angle around the Y-axis
    *roll += ((float)gyr_data[1] / GYROSCOPE_SENSITIVITY) * dt;    // Angle around the Y-axis
    *yal +=  ((float)gyr_data[2] / GYROSCOPE_SENSITIVITY) * dt;    // Angle around the z-axis

    // Turning around the X axis results in a vector on the Y-axis
    pitch_acc = atan2f((float)acc_data[0], (float)acc_data[2]) * 180 / M_PI;
